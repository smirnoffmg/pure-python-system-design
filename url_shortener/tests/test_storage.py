...
from url_shortener.storage import BaseStorage


@pytest.mark.asyncio
async def test_storage_idempt(storage: BaseStorage):
    short_url_1 = await storage.get_short_url("google.com")
    short_url_2 = await storage.get_short_url("google.com")
...
async def test_storage_equiv(test_url: str, storage: BaseStorage):
    short_url = await storage.get_short_url(test_url)
...
async def test_concurrent_operations(storage: BaseStorage, tasks_n: int):
    tasks = [storage.get_short_url(f"http://example.com/{i}") for i in range(tasks_n)]
